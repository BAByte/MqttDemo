...
对于mosquitto服务器的选取是因为调试方便，配置简单，轻量，该服务器的并发等什么的，下面会有分析。

# 对实现方案的要求：

## 实时性和服务质量如何？

实时性，连续发1000条，一个 “1” 字符串数据，大部分情况是2秒内收完。有时候超过，可能是我的网络不稳定。
...
在我测试时连接是非常快速的，即使是网络状态发生了改变，重新联后自动重连没有出现失败（可能是我测试的次数不够多？），即使是服务器重启，客户端也能很好的得到重连。

MQTT协议在连接一定的时长后，会自动断开，但是Paho库是提供了心跳机制去解决这问题。

弱网络下我不知道怎么控制这个变量，所以我找了一些资料。

大量发数据时会出现丢包（我重启了服务器后，丢包问题出现就少了，但是当我发了一万条消息后，再大量发送消息，又会出现丢包，这个可能和MQTT服务器配置有关，可能对消息做了一些限制）。
...
测试注意：当mqtt客户端向服务端发送信息出现丢包的情况

- 弱网络环境下会丢包，这点MQTT提供了3种服务质量，上面有说的，可以单独针一个事件设置服务质量，能得到改善。测试的话，如果像我一样没有改mosquitto服务器的任何配置，你测试的时候要是第一次大量发送信息，后面可能丢包的情况就又会出现。
...
可以看到自发自收和两个不同终端之间发送接收差别是很大的，所以上面的大佬的测试应该是属于在自己电脑上自发自收了。
...
- 遇到的一个坑：MqttConnectOptions设置isAutomaticReconnect()为true时可自动重连， 但多个相同的ClientID同时创建连接时会无限的连接中断和自动连接,所以这里我使用的是设备的SN码作为ClientID。

- 网络环境变化时是否会影响自动重连？当网络断开，网络又重新连接上的时候：这个会有对应回调，但是重新连接服务器后，订阅事件不会自动重新订阅，所以可以在自动重连回调中去判断是否需要重新订阅。
